...
class Config:
    ...
    admin_ids: List[int]
    database_path: str
    max_file_size: int          # bytes, incoming files (Bot API download cap = 20 MB)
    temp_directory: str
    font_path: str
    ffmpeg_path: str
    ...
    def from_env() -> "Config":
        ...
        max_size = int(os.getenv("MAX_FILE_SIZE", str(20 * 1024 * 1024)))
        max_size = max(1024 * 1024, min(max_size, 20 * 1024 * 1024))  # Bot API download limit
        ...
        return Config(
            ...
            admin_contact_url=os.getenv("ADMIN_CONTACT_URL", "https://t.me/").strip(),
...
# --------------------------------------------------------------------------
# UI screens & keyboards (mirror the reference screenshots)
# --------------------------------------------------------------------------
...
def main_menu_kb() -> InlineKeyboardMarkup:
    return kb([
        [btn("➕ Add Watermark", "nav:add"), btn("⚙️ Watermark Settings", "nav:hub")],
        [btn("🖼️ Process Image", "proc:image"), btn("📄 Process PDF", "proc:pdf")],
        [btn("🎥 Process Video", "proc:video"), btn("⚡ Batch Processing", "nav:batch")],
        [btn("📊 My Settings", "nav:myset"), btn("❓ Help", "nav:help")],
        [btn("ℹ️ About", "nav:about"),
         btn("🧑‍ Contact Admin for VIP Plan", url=CFG.admin_contact_url)],
...
def settings_hub_kb() -> InlineKeyboardMarkup:
    return kb([
        [btn("🎥 Video Watermark", "cat:video"), btn("🖼️ Image Watermark", "cat:image")],
        [btn("📄 PDF Watermark", "cat:pdf"), btn("🌐 Global Defaults", "cat:global")],
...
def main_menu_text() -> str:
    return (
        ...
        "\n<blockquote expandable>⚡ <b>Channel Batch Watermarking</b> is an automated tool "
...
def settings_hub_text() -> str:
    return (
        ...
        "<blockquote expandable>Select a media category below to configure watermarks "
...
def rate_limited(uid: int) -> bool:
    now = time.time()
    dq = RATE.setdefault(uid, deque())
    while dq and now - dq[0] > 60:
        dq.popleft()
    dq.append(now)
    return len(dq) > limit_rate_per_min()
...
async def download_update_file(update: Update) -> Tuple[str, str, int]:
    ...
    if size > CFG.max_file_size:
        raise WatermarkError(
            ...
            f"Limit is {human_size(CFG.max_file_size)}. "
            "Telegram's Bot API also caps downloads at 20 MB."
        )
    ...
    try:
        ...
        if size > 50 * 1024 * 1024:
            await status.edit_text(
                "✅ Processed, but the result exceeds Telegram's 50 MB upload "
                "limit, so it cannot be sent here. The file was kept in the "
                "server temp directory.", parse_mode=ParseMode.HTML)
            return
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("🧭", reply_markup=menu_reply_kb())  # type: ignore[union-attr]
...
async def run_batch(update: Update, context: ContextTypes.DEFAULT_TYPE,
    ...
    for i, (file_id, ext) in enumerate(files, 1):
        ...
        try:
            ...
            if (file.file_size or 0) > CFG.max_file_size:
                raise WatermarkError("file too large")
            await file.download_to_drive(path)
    ...
    if len(results) == 1:
        ...
        if len(data) <= 50 * 1024 * 1024:
            ...
            await status.edit_text("✅ Done, but result exceeds Telegram's 50 MB limit.")
    else:
        ...
        if size <= 50 * 1024 * 1024:
            ...
            await status.edit_text(
                ...
                "exceeds Telegram's 50 MB upload limit. Files kept in server temp dir.")
    await status.delete()
...
# Max incoming file size in bytes (Bot API download cap is 20 MB = 20971520)
...
| `MAX_FILE_SIZE`   | incoming limit in bytes (Bot API download cap 20 MB)| `20971520`         |
...
def run_selftest() -> None:
    ...
    CFG = Config(
        ...
        video_timeout=180, process_timeout=120,
    )
